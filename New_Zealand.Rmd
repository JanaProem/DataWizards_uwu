...
ggplot(data_kreis, aes(x="", y=desc(n), fill=criteria)) +
  ...
  scale_fill_manual(values = c("Es gibt weniger als 2 Frauen" = "#3C5186",
      ...
      geom_text(aes(label = paste0(n)), position = position_stack(vjust = 0.5), color = "white") +
  ...
  theme(
        ...
        plot.title = element_text(face = "bold"), ## funktioniert nicht???
...
bechdel_genre %>%
  ggplot(aes(x = n, y = reorder(genre, n), fill=Ergebnis))+
    ...
    scale_fill_manual(values = c(durchgefallen = "#ff696c", bestanden = "#5371bd")) +
    theme(
...
## Fazit
Obwohl Neuseeland bei dem Gleichstellungsindex insgesamt sehr gut ausfällt, fallen die in dem Land produzierten Filme generell beim Bechdel-Test durch – hauptsächlich, weil die Frauenrollen nicht miteinander reden. Lediglich Drama-Filme bestehen den Test häufiger.
Somit spiegeln die Filme von Neuseeland nicht direkt seinen Gleichberechtigungsstandard wider.
